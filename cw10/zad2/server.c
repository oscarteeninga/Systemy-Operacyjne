...
int main(int argc, char *argv[]) {
    if (argc != 3) error_exit("Pass: udp port, unix socket path");

    init_server(argv[1], argv[2]);

    struct epoll_event event;
    while (1) {
        if (epoll_wait(epoll, &event, 1, -1) == -1) error_exit_errno();
...
void handle_call(int sock)
{
    ...
    case REGISTER: {
        ...
        if (clients[i].name == NULL) error_exit_errno();
...
sock_msg_t get_msg(int sock, struct sockaddr *addr, socklen_t *addr_len)
{
    ...
    if (recv(sock, buff, head_size, MSG_PEEK) < head_size)
        error_exit("Uknown message from client");
