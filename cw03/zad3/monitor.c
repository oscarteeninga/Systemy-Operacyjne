...
int monitor(char *file_name, int interval, int time, int mode, rlim_t lim_cpu, rlim_t lim_mem);
...
int main(int argc, char *argv[]) {
    ...
    while (lines) {
        ...
        if (sscanf(lines, "%255s %d", file_name, &interval) == 2) {
            child_count += monitor(file_name, interval, monitoring_time, mood, cpu, mem);
        } else {
    ...
    for (int i = 0; i < child_count; i++) {
        ...
        if (getrusage(RUSAGE_CHILDREN, &r_usage)) {
            printf("Could not get usage of all children\n");
            exit(1);
        }
        ...
        if (getrusage(RUSAGE_CHILDREN, &r_usage_new)) {
            printf("Could not get usage of all children\n");
            exit(1);
        }
...
int monitor(char *file_name, int interval, int time, int mode, rlim_t lim_cpu, rlim_t lim_mem) {
    ...
    if (child_pid == 0) {
        ...
        cpu_limit.rlim_cur = cpu_limit.rlim_max = lim_cpu;
        mem_limit.rlim_cur = mem_limit.rlim_max = lim_mem;

        if (setrlimit(RLIMIT_CPU, &cpu_limit)) {
            printf("Could not set time limit for cpu\n");
            exit(1);
        }
        if (setrlimit(RLIMIT_AS, &mem_limit)) {
            printf("Could not set limit for memory\n");
            exit(1);
        }

        if (getrlimit(RLIMIT_CPU, &cpu_limit)) {
            printf("Could not get time limit for cpu\n");
            exit(1);
        }
        if (getrlimit(RLIMIT_AS, &mem_limit)) {
            printf("Could not get limit for memory\n");
            exit(1);
        }
        if (mode == 1) {
...
void watch_2 (char *file_name, int interval, int time) {
    ...
    while ((elapsed += interval) <= time) {
        ...
        if (stat.st_mtime > last_mod || n == 0) {
            ...
            if (child_pid == 0) {
                if(execlp("cp", "cp", file_name, file_name_date, NULL) == -1) exit(n);
            } else if (child_pid == -1) {
